...
import { addSubject, deleteSubject, resetDeleteState, resetFormValues, resetUIstate, setDeleteId,
    setDeleteOpen, setEditId, setFormValues, setSubject, setOpenForm, setSearchItem, updateSubject,
} from "./SubjectSlice";
...
const AddSubject = () => {
    const { list: subjects = [], formValues, openForm, searchItem, deleteOpen, deleteId, editId } 
        = useSelector((state) => state.subjectStore);
    ...
    const postData = (values, resetForm) => {
        ...
        axios.post("https://generateapi.techsnack.online/api/subject", data, {
    ...
    const deleteData = () => {
        ...
        .then((res) => {
            ...
            if (res.status === 200 || res.status === 204) {
                ...
                ShowSnackbar("Subject Deleted Successfully !", "error");
            }
        })
        .catch((err) => {
    ...
    const patchData = (id, values, resetForm) => {
        ...
        .then((res) => {
            ...
            if (res.status === 200 || res.status === 204) {
                dispatch(updateSubject({ _id: id, ...values }));
    ...
    const handleEdit = (item) => {
        ...
        dispatch(setFormValues({ subjectName: item.subjectName, subjectCode: item.subjectCode,
            status: item.status,
        }),
        );
    };
    ...
    return (
        <>
            <Box>
                            ...
                            <Form className="subject-form">
                                ...
                                >
                                    ...
                                    sx={{ display: "flex", flexDirection: "column", gap: 1, flex: 1 }}>
                                        ...
                                        <Field name="subjectCode" id="subjectCode"
                ...
                <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" },
                        justifyContent: "center", alignItems: { xs: "stretch", sm: "center" }, my: 2,
                        gap: { xs: 1, sm: 0 },
                    }}
                >
                    {/* Search Field */}
                ...
                {!isMobile ? (
                    ...
                    >
                        <Table sx={{ borderCollapse: "separate", borderSpacing: 0 }}>
                            ...
                            >
                                <TableRow>
                                    {[ "#", "Subject Name", "Subject Code", "Status", "Actions" ].map((h) => (
                    ...
                    {filteredSubject.map((item, index) => (
                        ...
                        >
                            ...
                            <CardContent sx={{ flexGrow: 1 }}>
                                ...
                                <Typography variant="body2">
                                    <b>Subject Code:</b> {item.subjectCode}
                                </Typography>
