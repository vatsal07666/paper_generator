...
const AddSubject = () => {
    const { list: topics = [], formValues, openForm, searchItem, deleteOpen, deleteId, editId,
    ...
    const validationSchema = Yup.object({
        ...
        languageName: Yup.string().required("Subject Code is Required*"),
        status: Yup.string().required("Status is Required*"),
    });

    const token = "ROorKmyDQ65fmCmm";
    ...
    const postData = (values, resetForm) => {
        const topicData = { topicName: values.topicName, languageName: values.languageName,
    ...
    const handleEdit = (item) => {
        ...
        dispatch(
            setFormValues({ topicName: item.topicName, languageName: item.languageName, status: item.status })
        );
    };

    /* ---------------- Search ---------------- */
    const filteredTopic = topics.filter((l) =>
        [l.topicName, l.languageName, l.status].join(" ").toLowerCase().includes(searchItem.toLowerCase()),
    );
        ...
        <>
            <Box>
                                ...
                                <Form className="topic-form">
                                    ...
                                    >
                                        <Box sx={{ display: "flex", flexDirection: "column", gap: 1, flex: 1 }}>
                                            <label htmlFor="topicName">Subject Name</label>
                                            <Field name="topicName" id="topicName" placeholder="Enter Subject Name" />
                                    ...
                                    >
                                        <Box sx={{ display: "flex", flexDirection: "column", gap: 1, flex: 1 }}>
                                            ...
                                            <Field name="languageName" id="languageName" as="select">
                                                <option value="" hidden> Select Language Name </option>
                                                {(languages || []).filter((l) => l.status === "Active")
                                                .map((l) => (
                                                    <option key={l._id} value={l.languageName}> 
                                                        {l.languageName} 
                                                    </option>
                                                ))}
                                            </Field>
                ...
                >
                    ...
                    >
                        <InputBase name="search" placeholder="Search Topic" value={searchItem ?? ""}
                            onChange={(e) => dispatch(setSearchItem(e.target.value))}
                            sx={{ paddingLeft: "40px", width: "100%", boxSizing: "border-box" }}
                        />
                        <SearchIcon sx={{ position: "absolute", left: 10, top: "50%", 
                ...
                {!isMobile ? (
                    ...
                    >
                        <Table sx={{ borderCollapse: "separate", borderSpacing: 0 }}>
                            ...
                            >
                                <TableRow>
                                    {[ "#", "Topic Name", "Language Name", "Status", "Actions", ].map((h) => (
                                    ...
                                    filteredTopic.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <TableCell style={{ textAlign: "center" }}>
                                                <span style={{
                                                        background: item.status === "Active" ? "#4caf50" : "#f44336",
                                                        ...
                                                        slotProps={{
                                                            tooltip: {
                                                                sx: { fontSize: "12px", px: 2,
                                                                    color: "#ef4444", background: "#ffddddff",
                        ...
                        {filteredTopic.map((item, index) => (
                            ...
                            >
                                ...
                                <CardContent sx={{ flexGrow: 1 }}>
                                    ...
                                    <Typography variant="body2">
                                        <b>Language Name:</b> {item.languageName}
                                    </Typography>
